...
Provider = {
	...
	encode: function(url, options, callback) {
		...
		this.check(url, function(err, url) {
			if (err) {
	...
	stats: {
		...
		hit: function(key, callback) {
			...
			client.incr(key_short2stats(key), callback)
		},

		get: function(key, callback) {
			client.mget(key_short2stats(key), key_short2long(key), function(err, data) {
				callback(err, data[0], data[1])
			})
		},
