	...
	if (!key) {
		...
		(details ? url_provider.stats.getDates : url_provider.stats.get)(key, function(err, hits, url) {
			if (err) {
				...
				hits = hits || 0
				var shortUrl = 'http://' + app.set('www_host') + '/' + key
...
function preExecute(req, res, next) {
	// Support for CORS
	var req_origin = req.header('Origin')
	res.header('Access-Control-Allow-Origin', (req_origin == 'null' || !req_origin) ? '*' : req_origin)
