...
function home(req, res, next) {
	...
	// Host "api.301.tl"
	if (host.match(/^api\./)) {
		// api.301.tl?longURL=XXX[&customKey=YYY][&generateNewKey=0|1] -> shorten URL
		if (req.query.longUrl) { // http://api.301.tl?longUrl=XXX
			var custom_key = (req.query.customKey && req.query.customKey.match(/^[a-zA-Z0-9]+$/)) ? req.query.customKey : null
			var new_key = req.query.generateNewKey == '1'
			url_provider.encode(req.query.longUrl, {key:custom_key, new:new_key}, function(err, key) {
				if (err) {
