...
function stats(req, res, next) {
	...
	if (!key) {
		...
		url_provider.stats.get(key, function(err, hits, url) {
			if (err) {
				exports.error(err, req, res, next)
			} else if (!url) {
				exports.error(new Error404('No long URL for key "' + key + '"'), req, res, next)
				...
				if (req.is('text')) {
					...
					res.send('<a href="' + shortUrl +'" title="' + url + '">' + req.domain + '/<strong>' + key + '</strong></a> has been hit ' + hits + ' times.', {'Content-Type':'text/html'})
				}
			}
		})
	}
